...
Both call the same `requestPortalToken()` in `api/arcgis-token.ts`, so the client
never knows which one answered.

**`api/arcgis-token.ts` must stay free of relative imports.** This package is
`"type": "module"`, so Node requires a file extension on relative imports.
An extensionless import resolves under Vite but fails to load on Vercel, and
the function then dies with an opaque `FUNCTION_INVOCATION_FAILED` before any
handler code runs — indistinguishable at a glance from a credentials problem,
but not one.
...
setting Production only leaves every preview deployment with the locator
offline:
...
| `FUNCTION_INVOCATION_FAILED`, or Vercel's own `{"error":{"code":"500"}}` | The function crashed at module load — a build or import problem, not a credentials one. Usual cause: a relative import in `api/arcgis-token.ts`. Note the shape — this app's errors are `{"error":"message"}` with a plain string, so a nested `error.code` object is always Vercel's, never ours. |
